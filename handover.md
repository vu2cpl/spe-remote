...
- The hardware lines work whether the amp's CPU is alive or not — that's the whole point. The 2026-05-15 client-consolidation work (Recent work item 12) was triggered by a Node-RED flow that wrongly assumed the WS path couldn't wake a powered-off amp because the *data* link was dead. The data link is irrelevant; DTR is a separate wire.
...
- `polling.amp_alive_threshold` 3s — applies to the most recent of CSV state OR RCU display frame (see item 13 in Recent work). RCU OFF→ON ticks at 1.5s, so 3s is a generous margin while still flipping `serial:"down"` within ~3s of amp power-off (FTDI link survives, but no more RCU frames come back).
