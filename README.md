...
| `tune_band:<band>` | Sweep the SPE manual's recommended in-band sub-band centers. **The radio rules the band**: the radio's slice freq is mapped to its ham band and that band is swept — the payload band (`160m`, `80m`, …, `6m`; also `auto` / `current` / empty) is overridden with a note if it disagrees, and only trusted when the radio's band can't be read. Auto-switches the amp to STBY, saves the operator's pre-sweep VFO freq + mode, hits each sub-band in turn, then restores the VFO and hands OPERATE back iff it was on at the start. |
...
| `tune_band:<band>` | Sweep the manual's sub-bands. The radio's current band rules — the payload band (e.g. `tune_band:20m`, or `auto`) is only trusted when the radio's band can't be read. Amp auto-STBY'd, OPERATE restored after |
